...
- [x] **Firecracker backend integration** — `src/egress/fc_net.rs` + `src/firecracker.rs`. Per-capsule TAP in `169.254.33.0/24` (separate range from Namespace's `.32`). Proxy binds on TAP host IP. zk-init configures eth0 + default route inside the guest from `ZK_FC_GUEST_IP`/`ZK_FC_HOST_IP`/`ZK_FC_GUEST_IFACE` env vars using busybox `ip` (Alpine minirootfs). FC config auto-overrides `enable_network=true` and `tap_name=<allocated>` when egress is set. Compile-verified on Linux x86_64; full end-to-end requires KVM-capable host (CI runner with `/dev/kvm`)
